...
@WebServlet("/RealTerm")
public class RealTermServlet extends HttpServlet {
		...
		try {
			...
			"jdbc:mysql://localhost:3306/javawebdb", "javaweb" , "javaweb" );
				...
				rs = stmt.executeQuery(
					"SELECT * FROM intercitybus where STARTTIME >= now() order by STARTTIME asc limit 3 ");
				
				out.println("<html><head><title>시외버스 시간표</title></head>");
